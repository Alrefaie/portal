...
class Survey(models.Model):
    ...
    def prepare_result(self, question, current_filters=None):
        ...
        # Sign
        if question.question_type == 'sign':
            result_summary = []
            for input_line in question.user_input_line_ids:
                if not (current_filters) or input_line.user_input_id.id in current_filters:
                    result_summary.append(input_line)
...
class UserInput(models.Model):
    ...
    def save_line_m2o(self, user_input_id, question, post, answer_tag):
        ...
        if record:
            #record.flush(['user_input_id', 'question_id', 'page_id', 'survey_id', 'skipped', 'answer_type', 'value_m2o', 'value_text'])
            record.write(vals)
        else:
            self.create(vals)
        return True
    ...
    def save_line_binary(self, user_input_id, question, post, answer_tag):
        ...
        if record:
            ...
            if attachment_value and record.attachment_id:
                #record.attachment_id.flush(['name', 'datas', 'store_fname', 'res_model', 'res_id', 'description'])
                record.attachment_id.write(attachment_value)
            #record.flush(['user_input_id', 'question_id', 'page_id', 'survey_id', 'skipped', 'answer_type', 'attachment_id'])
            record.write(vals)
        else:
            # Create New Record
            ...
            if attachment:
                #attachment.flush(['res_id'])
                attachment.write({'res_id': record.id})
                main_answer = self.env['survey.user_input'].with_user(self.env['res.users'].browse(SUPERUSER_ID)).browse(user_input_id)
    ...
    def save_line_name(self, user_input_id, question, post, answer_tag):
        ...
        if old_uil:
            #old_uil.flush(['user_input_id', 'question_id', 'survey_id', 'skipped', 'answer_type'])
            old_uil.write(vals)
        else:
            old_uil.create(vals)
        return True
    ...
    def save_line_sign(self, user_input_id, question, post, answer_tag):
        ...
        if old_uil:
            #old_uil.flush(['user_input_id', 'question_id', 'survey_id', 'skipped', 'answer_type', 'page_id', 'img_sign'])
            old_uil.write(vals)
        else:
            old_uil.create(vals)
        return True
