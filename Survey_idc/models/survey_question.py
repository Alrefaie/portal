...
class SurveyQuestion(models.Model):
    ...
    @api.onchange('validation_mobile')
    def _onchange_validation_mobile(self):
        if self.validation_mobile:
            self.validation_required = False
    ...
    def validate_mobile(self, post, answer_tag):
        ...
        if answer and self.validation_required:
            ...
            if not (self.validation_min_float_value <= len(answer) <= self.validation_max_float_value):
                errors.update({answer_tag: self.validation_error_msg})
        return errors
